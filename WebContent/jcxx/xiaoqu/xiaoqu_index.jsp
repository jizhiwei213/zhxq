...
<html>
	...
	<body>
		<table id="datagrid_<s:property value='rand'/>" toolbar="#tb_datagrid_<s:property value='rand'/>" fitColumns="true">
			<thead>
				<tr>
					...
					<th field="xiaoquNo" width="150">小区编号</th>
					<th field="xiaoquAddress" width="150">小区地址</th>
					<th field="xiaoquLianxiren" width="150">联系人</th>
					<th field="xiaoquLianxidianhua" width="150">联系人电话</th>
					<th field="fuzeren" width="150">公司负责人</th>
					<th field="fuzerentel" width="150">公司负责人联系电话</th>
					<th field="shenname" width="150">所在省</th>
					<th field="shiname" width="150">市</th>
					<th field="quname" width="150">区/县</th>
					<th field="wygsname" width="150">所属物业公司</th>
				</tr>
			</thead>
		</table>
		...
		<div id="tb_datagrid_<s:property value='rand'/>">
			...
			<div>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;小区名称：</span>
				<input style="width:150px;height:24px;" type="text" id="xiaoquNameQ" name="xiaoquNameQ" class="easyui-validatebox" />
				<span>&nbsp;&nbsp;&nbsp;&nbsp;小区编号：</span>
				<input style="width:150px;height:24px;" type="text" id="xiaoquNoQ" name="xiaoquNoQ" class="easyui-validatebox" />
				<span>&nbsp;&nbsp;&nbsp;&nbsp;小区地址：</span>
				<input style="width:150px;height:24px;" type="text" id="xiaoquAddressQ" name="xiaoquAddressQ" class="easyui-validatebox" />
		...
		<script type="text/javascript">
			$(function(){
				$("#datagrid_<s:property value='rand'/>").datagrid({
					url:'<n:base/>/jcxx/xiaoquList.action',
					title: '小区信息',
					loadMsg: '数据正在加载中请稍候......',
			        striped: true,
			        fit: true,
		...
		<script type="text/javascript">
			function search_<s:property value="rand"/>(){
				$('#datagrid_<s:property value='rand'/>').datagrid('load',
						{       
					"xiaoquQuery.xiaoquNameQ": $('#xiaoquNameQ').val(),
					"xiaoquQuery.xiaoquNoQ":$('#xiaoquNoQ').val(),
					"xiaoquQuery.xiaoquAddressQ":$('#xiaoquAddressQ').val()
	    				});
			}
			function openUserDialog_<s:property value="rand"/>(action){
		        ...
		        $('#edit_<s:property value="rand"/>').dialog({
					...
					height : 'auto',
					modal : true,
					href:'<n:base/>/jcxx/xiaoquEdit.action?xiaoquModel.id='+id,
					iconCls:'icon-edit',
					resizable :true,
					buttons:[{
						text:'确定',
			...
			function deleteUser_<s:property value='rand'/>(){
		        ...
		        if(keys.length>0){
					...
					$.messager.confirm('消息提醒','您确定删除<span style="color:red">'+rows.join('、')+'</span>吗？',function(r){
						if(r){
							$.getJSON("<n:base/>/jcxx/xiaoquDelete.action",{"keys":ids.join(',')},function(data){
