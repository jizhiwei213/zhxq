	...
	<body>
    	<table id="user_data_<s:property value="rand"/>">
			<thead>
				<tr>
					...
					<th field="xiaoquName" width="100">管辖小区</th>
					<th field="roles" width="100">角色</th>
		...
		<div id="tb_<s:property value="rand"/>" style="padding:3px;">
			<div>
				...
				<span>身份证号 ：</span>
				<input style="width:150px;height:24px;" type="text" id="userCode" name="userCode" class="easyui-validatebox" />
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('#user_data_<s:property value="rand"/>').datagrid({
			        ...
			        fit: true,
			        fitColumns: true,
			        url: '<n:base/>/jcxxyhgl/gsqUserList.action',
			...
			function openUserDialog_<s:property value="rand"/>(action){
		        ...
		        $('#edit_<s:property value="rand"/>').dialog({
					...
					height : 'auto',
					modal : true,
					href:'<n:base/>/jcxxyhgl/gsqUserEdit.action?userModel.id='+id,
					iconCls:'icon-edit',
					resizable :true,
					buttons:[{
						text:'确定',
			...
			function stopUser_<s:property value="rand"/>(){
		        ...
		        if(keys.length>0){
					...
					$.messager.confirm('消息提醒','您确定禁用<span style="color:red">'+rows.join('、')+'</span>吗？',function(r){
						if(r){
							$.getJSON("<n:base/>/jcxxyhgl/gsqUserStop.action",{"keys":ids.join(',')},function(data){
			...
			function startUser_<s:property value="rand"/>(){
		        ...
		        if(keys.length>0){
					...
					$.messager.confirm('消息提醒','您确定启用<span style="color:red">'+rows.join('、')+'</span>吗？',function(r){
						if(r){
							$.getJSON("<n:base/>/jcxxyhgl/gsqUserStart.action",{"keys":ids.join(',')},function(data){
								$("#user_data_<s:property value='rand'/>").datagrid('reload');
								var errorNames=[];
								var errors=data.errors;
								for(var i=0;i<errors.length;i++){
			...
			function deleteUser_<s:property value='rand'/>(){
		        ...
		        if(keys.length>0){
					...
					$.messager.confirm('消息提醒','您确定删除<span style="color:red">'+rows.join('、')+'</span>吗？',function(r){
						if(r){
							$.getJSON("<n:base/>/jcxxyhgl/gsqUserDelete.action",{"keys":ids.join(',')},function(data){
								$("#user_data_<s:property value='rand'/>").datagrid('reload');
								var errorNames=[];
								var errors=data.errors;
								for(var i=0;i<errors.length;i++){
