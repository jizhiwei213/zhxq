...
<html>
	...
	<body>
		<table id="datagrid_<s:property value='rand'/>" toolbar="#tb_datagrid_<s:property value='rand'/>" fitColumns="true">
			<thead>
				<tr>
					...
					<th field="shenname" width="150">所在省份</th>
					<th field="shiname" width="150">所在省份市</th>
				</tr>
			</thead>
		</table>
		...
		<div id="tb_datagrid_<s:property value='rand'/>">
			...
			<div>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;名称：</span>
				<input style="width:150px;height:24px;" type="text" id="shiNameQ" name="shiNameQ" class="easyui-validatebox" />
		...
		<script type="text/javascript">
			$(function(){
				$("#datagrid_<s:property value='rand'/>").datagrid({
					url:'<n:base/>/jcxx/zhxqQuList.action',
					title: '区/县管理',
					loadMsg: '数据正在加载中请稍候......',
			        striped: true,
			        fit: true,
		...
		<script type="text/javascript">
			function search_<s:property value="rand"/>(){
				$('#datagrid_<s:property value='rand'/>').datagrid('load',
						{       
					"zhxqShenShiquQuery.shiNameQ": $('#shiNameQ').val()
	    				});
			}
			function openUserDialog_<s:property value="rand"/>(action){
		        ...
		        $('#edit_<s:property value="rand"/>').dialog({
					...
					height : 'auto',
					modal : true,
					href:'<n:base/>/jcxx/zhxqQuEdit.action?zhxqQuModel.id='+id,
					iconCls:'icon-edit',
					resizable :true,
					buttons:[{
						text:'确定',
			...
			function deleteUser_<s:property value='rand'/>(){
		        ...
		        if(keys.length>0){
					...
					$.messager.confirm('消息提醒','您确定删除<span style="color:red">'+rows.join('、')+'</span>吗？',function(r){
						if(r){
							$.getJSON("<n:base/>/jcxx/zhxqQuDelete.action",{"keys":ids.join(',')},function(data){
